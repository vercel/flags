...
- Run Edge Middleware for requests to `/pricing`, and pass `pricingFlags` to the `precompute` function
- Adjust the rewrite in Edge Middleware to rewrite requests from `/pricing` to `/pricing/[pricingCode]`
...
Adjust the rewrite in Edge Middleware to include the `precomputed` segment.

Note that you will need to manually maintain the paths in Edge Middleware for which the rewrite should run.
...
To use this pattern, you need to adjust Edge Middleware:
