...
At a high level we use Edge Middleware and SvelteKit's `reroute` hook to rewrite the incoming request between static versions of the page.
...
You therefore must use create a cookie (for example with a random id) as part of Edge Middleware and `reroute`. Here's how you do it within middleware (similarly for reroute):
...
`visitorId` cookie. Edge Middleware (or `reroute` indirectly via the API call) will generate
...
`x-visitorId` request header from Edge Middleware and the API,
