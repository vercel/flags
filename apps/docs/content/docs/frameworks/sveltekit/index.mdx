...
<Callout title="Flags SDK agent skill" type="info">
  Install the Flags SDK agent skill with `npx skills add vercel/flags@flags-sdk` so
  your AI coding assistant follows the recommended workflows. See the [agent skill
  guide](/docs/agent-skill).
</Callout>
