...
# Determines the region your data lives in:
...
| -------------------------- | ----------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `POSTHOG_PROJECT_API_KEY`  | Yes                     | `postHogAdapter`            | Your project API key (`phc_...`). Settings > Project > Project API Key.                                                          |
| `POSTHOG_HOST`             | Yes                     | `postHogAdapter`, Explorer  | The regional API host, e.g. `https://us.i.posthog.com` or `https://eu.i.posthog.com`.                                            |
| `POSTHOG_SECRET_KEY`       | No                      | `postHogAdapter`            | Feature flags secret key (`phs_...`). Setting it opts the default adapter into [local evaluation](#local-evaluation).             |
| `POSTHOG_PERSONAL_API_KEY` | For the Flags Explorer  | `getProviderData`           | Personal API key (`phx_...`). Settings > User > Personal API keys.                                                               |
| `POSTHOG_PROJECT_ID`       | For the Flags Explorer  | `getProviderData`           | Your numeric project id, e.g. `521742`. Settings > Project > Project ID.                                                          |
...
Set a PostHog project secret key to opt the default adapter into local evaluation:
...
This is the only thing that switches evaluation modes — the default adapter enables
local evaluation exactly when `POSTHOG_SECRET_KEY` is present.
