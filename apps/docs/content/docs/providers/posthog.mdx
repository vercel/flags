...
One PostHog flag can back multiple SDK flags by using a suffix after a `.` in the
key (for example `my-flag.enabled` and `my-flag.payload` both read the PostHog flag
`my-flag`).
