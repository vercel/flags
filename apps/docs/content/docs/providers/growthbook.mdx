...
| `EXPERIMENTATION_CONFIG`            | Optional. Used when installed through Vercel Marketplace (replaces GROWTHBOOK_GLOBAL_CONFIG_CONNECTION_STRING) |
...
The adapter can load feature configuration from [Vercel Global Config](https://vercel.com/docs/storage/edge-config) to lower the latency of feature flag evaluation.
