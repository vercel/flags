...
Run `pnpm check:deps` to check dependency consistency across all pnpm workspace
packages. The Quality workflow runs the same check on pull requests.

Run `pnpm fix:deps` to align manifests, then `pnpm install` to refresh the lockfile.
...
and run `pnpm fix:deps`. Do not remove that exception when upgrading other apps.
