...
- **Targeting attributes**: `split`, `rollout`, and `rules add` take `--by <entity>.<attribute>` (and `--condition <entity>.<attribute>:<op>:<value>`). The path must be an entity attribute configured under Flags → Entities in the dashboard, and it must exist on the object the flag's `identify()` returns. For the `identify` in [User targeting](#user-targeting) that is `--by user.id`. When the attribute is missing at runtime, the flag serves `--default-variant` and nothing is bucketed. No error is raised, so verify with `evaluations`.
