...
describe('FlagNetworkDataSource', () => {
  ...
  describe('primed message support', () => {
    it('should send X-Revision header from initial datafile revision', async () => {
      let capturedHeaders: Headers | null = null;
              ...
              start(controller) {
                controller.enqueue(
                  new TextEncoder().encode(
                    `${JSON.stringify({
                      type: 'datafile',
                      data: { projectId: 'test', definitions: {}, revision: 7 },
                    })}\n`,
                  ),
                );
                request.signal.addEventListener('abort', () => {
      ...
      expect(capturedHeaders!.get('X-Revision')).toBe('5');
    ...
    it('should stop polling on primed message', async () => {
      let pollCount = 0;
      ...
      const pollCountAfterPrimed = pollCount;

      await new Promise((r) => setTimeout(r, 200));

      expect(pollCount).toBeLessThanOrEqual(pollCountAfterPrimed + 1);
